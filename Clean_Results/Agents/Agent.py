...
import Clean_Results.Agents.Models as Models
import Clean_Results.Agents.hyperparameters as hp
...
class DQN_Agent():
    ...
    def learn(self):
        ...
        if self.double_q:
            ...
            if(len(self.memory) < self.batch_size): target_q = [0]*len(self.memory)
            else: target_q = [0]*self.batch_size
            index = 0
            for TQ in target_q:
