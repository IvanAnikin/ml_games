...
def single_round(EPISODES = 25000, LEARNING_RATE = 0.1, epsilon = 0.5, end_epsilon_decaying_position = 2, DISCOUNT = 0.95, DISCRETE_OS_SIZE = [20, 20], SHOW_EVERY = 3000, STATS_EVERY=50):
			...
			for end_epsilon_decaying_cycle in range(len(END_EPSILON_DECAYING_POSITIONS)):
				for discount_cycle in range(len(DISCOUNTS)):
					for discrete_os_size_cycle in range(len(DISCRETE_OS_SIZES)):
						...
						print(NAME)
						start = time.time()
						stats_ep_rewards = single_round(LEARNING_RATE=learning_rate,
																	epsilon=epsilon,
